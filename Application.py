...
class Application:
    ...
    def setup_gui(self):
        ...
        # Главное окно
        with dpg.window(label="Главное окно", width=400, height=400):
            dpg.add_button(
        ...
        dpg.create_viewport(title="Omni", width=1215, height=400)
        dpg.setup_dearpygui()
    ...
    def generate_sum_table_docx(self):
        if self.re_ is None:
            ...
            Logger.info('Суммарная таблица сигналов в docx создана')
    ...
    def renew_sum_table_latex(self):
        if self.re_ is None:
            ...
            self.re_.renew_sum_table_latex()
            Logger.info('Суммарная таблица сигналов приложения А в РЭ обновлена')
    ...
    def start_device_task(self):
        ...
        if device_name and self.device_manager.set_current_device(device_name):
            ...
            #threading.Thread(target=self.device_task, daemon=True).start()

    #def device_task(self):
        """Фоновая задача устройства"""
        #device = self.device_manager.current_device
        #Logger.info(f"Начата работа с {device['name']}")

    def toggle_log_level(self, sender, app_data, user_data):
